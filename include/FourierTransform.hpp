...
template <typename T> class FourierTransform {
    ...
    FourierTransform(int n, map<int,T> f, string mode, int nthreads) {
        ...
        this->mode = mode;
        this->f = f;
        if(this->nfact % nthreads != 0){
            cout << "Number of threads must be a divisor of " << this->nfact << endl;
            exit(1);
        }
    ...
    void build_inv_ft(SnVector v, size_t order){
        ...
        int k = 0;
        this->invFT[to_int(inverse(p))*10 + order] = v[k];
        k++;
